...
export class GeminiService {
   ...
   */
  public async generateQuery(systemInstruction: string, schemaMetadata: string, userQuestion: string): Promise<GeminiResponse> {
    ...
    try {
      ...
      logger.info(`[STAGE 1: Query Generation] Received response from Gemini API in ${elapsed}ms`);

      const responseText = response.text;
      if (responseText && responseText.trim().length > 0) {
        logger.info(`--------------------------------------------------`);
        logger.info(`[STAGE 1: Query Generation] RAW GEMINI RESPONSE: ${responseText}`);
        logger.info(`--------------------------------------------------`);
      ...
      logger.error(`[STAGE 1: Query Generation] Error calling Gemini API: ${e.message}`);
   ...
   */
  public async generateFormattedResponse(
    ...
    try {
      ...
      logger.info(`[STAGE 2 DEBUG] Response Received from Gemini API in ${sdkDuration} ms`);

      const responseText = response.text;
      if (responseText && responseText.trim().length > 0) {
        logger.info(`--------------------------------------------------`);
        logger.info(`[STAGE 2: Response Generation] RAW GEMINI RESPONSE: ${responseText}`);
        logger.info(`--------------------------------------------------`);
        return this.parseGeminiResponseText(responseText);
      } else {
      ...
      const userFriendlyError = this.formatGeminiErrorMessage(e);
      logger.error(`[STAGE 2 DEBUG] Gemini Request Failed: ${e.message}`);
      throw new Error(userFriendlyError);
    }
  }
   ...
   */
  public async generateChatTitle(firstUserQuestion: string): Promise<string> {
    ...
    try {
      ...
      logger.warn(`AI title generation failed, using fallback: ${e.message}`);
    }
  ...
  private parseGeminiResponseText(responseText: string): GeminiResponse {
    try {
      const cleanJson = this.cleanMarkdownCodeBlocks(responseText);
      return JSON.parse(cleanJson) as GeminiResponse;
    } catch (e: any) {
      logger.error(`Failed to parse Gemini response JSON: ${e.message}`);
  ...
  private formatGeminiErrorMessage(throwable: any): string {
    if (!throwable) return 'An error occurred while communicating with Gemini API.';
